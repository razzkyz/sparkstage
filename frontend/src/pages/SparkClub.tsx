...
function SparkClubResults({ filteredProducts, loading, resetSignal, onPrefetchProduct, onAddToCart }: SparkClubResultsProps) {
...
const SparkClub = () => {
  ...
  useEffect(() => {
    if (error) {
      showToast('error', error instanceof Error ? error.message : 'Failed to load spark club data');
    }
  }, [error, showToast]);
          ...
          {error ? (
            <div className="mb-8 rounded-xl border border-red-500/20 bg-red-500/10 p-6 text-center">
              <p className="text-sm text-red-700 mb-4">
                {error instanceof Error ? error.message : 'Failed to load spark club data'}
              </p>
              <button
                type="button"
                onClick={() => {
