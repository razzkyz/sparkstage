...
function ServiceShopResults({
  filteredProducts,
  loading,
  resetSignal,
  onPrefetchProduct,
  onAddToCart,
}: ServiceShopResultsProps) {
    ...
    <>
      ...
      {totalProducts > 0 ? (
        <div className="mt-14 flex flex-col items-center gap-4">
          <p className="text-sm text-gray-500">
            Page {page} of {totalPages} ({totalProducts} services)
          </p>
...
const ServiceShop = () => {
  useSeo({
    title: "Service · Stage 55",
    description:
      "Discover our beauty and grooming services at Stage 55.",
    canonical: `${window.location.origin}/service`,
  });
  ...
  const loading =
    (productsLoading || retailCategoriesLoading) &&
    products.length === 0;
  ...
  // Service categories from retail_categories table (department = 'service')
  // Only show parent categories (main categories, not subcategories)
  // Sort by display_order
  const serviceCategoriesFlat = useMemo(() => {
    return retailCategories
      .filter((c) => c.department === "service" && c.is_active && c.parent_id === null)
  ...
  const activeSubcategories = useMemo(() => {
    ...
    const selectedCategory = serviceCategoriesFlat.find(
    ...
    return retailCategories
      .filter((c) => c.department === "service" && c.is_active && c.parent_id === selectedCategory.id)
      .sort((a, b) => (a.display_order || 999) - (b.display_order || 999));
  }, [activeCategory, serviceCategoriesFlat, retailCategories]);

  // Filter products to only include service department products
  const serviceProducts = useMemo(
    () =>
      products.filter((p) => {
        // Include products that belong to service department categories
        if (p.retail_category_id) {
          const category = retailCategories.find((c) => c.id === p.retail_category_id);
          return category?.department === 'service' && category?.is_active;
        }
        return false;
      }),
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (currentActiveCategory !== "all") {
      const cat = serviceCategoriesFlat.find(
              ...
              <div className="w-full mt-3 mb-3">
                <div className="mx-auto w-fit max-w-full overflow-x-auto category-scroll px-4 sm:px-6">
                  <div className="flex items-center space-x-6 md:space-x-8 pb-2">
                    ...
                    {serviceCategoriesFlat.map((category) => {
