...
const TicketsManagement = () => {
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    const matchesEvent =
      eventFilter === "" ||
      eventFilter === "all" ||
      ticket.tickets.name.toLowerCase().includes(eventFilter);
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-blue-700">
              Filter default menampilkan hanya tiket yang sudah dipindai. Data
              diperbarui secara otomatis saat ada perubahan.
            </p>
          </div>
          <button
            onClick={() => refetch()}
          ...
          >
            <span
              className={`material-symbols-outlined text-sm ${isFetching ? "animate-spin" : ""}`}
            >
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900">
            {statusFilter === "used"
              ? "Tiket yang Sudah Discan"
              : statusFilter === "active"
                ? "Tiket Belum Discan"
                : "Semua Tiket"}
          </h3>
          <div className="text-sm text-gray-600">
            Menampilkan {paginatedTickets.length} dari {filteredTickets.length}{" "}
            tiket (Halaman {currentPage} dari {totalPages || 1})
          </div>
        </div>
        {isLoading ? (
