...
const StockAdjustments = () => {
  ...
  const [editingAdjustment, setEditingAdjustment] = useState<
    (typeof adjustmentList)[0] | null
  >(null);

  const offset = (currentPage - 1) * ITEMS_PER_PAGE;
  const { data, isLoading, error, refetch } = useStockAdjustmentList(
    ITEMS_PER_PAGE,
    offset,
  );
  ...
  const handleCreateSuccess = (adjustmentNumber: string) => {
    ...
    showToast(
      "success",
      `Stock adjustment ${adjustmentNumber} berhasil ${action} dan stock telah diupdate!`,
    );
    ...
    >
      <section className="flex flex-col gap-6">
        ...
        {isLoading ? (
          ...
          <>
            <StockAdjustmentTable data={adjustmentList} onEdit={handleEdit} />

            {totalPages > 1 && (
              <div className="mt-6 flex flex-col items-center gap-4">
                <p className="text-sm font-sans text-gray-500">
                  Halaman {currentPage} dari {totalPages} ({totalCount}{" "}
                  adjustment)
                </p>
