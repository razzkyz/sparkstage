...
async function fetchProductRetailSummaries(signal?: AbortSignal): Promise<ProductRetail[]> {
  const query = supabase
    ...
    .select(`
      ...
      retail_categories(id, department, name, slug)
    `)
    .eq('is_active', true)
  ...
  return (data ?? []).map((row) => {
    ...
    const rCat = Array.isArray(row.retail_categories) ? row.retail_categories[0] ?? null : row.retail_categories ?? null;
    ...
    return {
      ...
      retail_category: row.retail_category as 'glam' | 'charmbar' | 'sparkclub' | null,
      categories: cat
        ? {
            id: cat.id,
            name: cat.name,
...
export async function fetchProductRetailDetail(id: number, signal?: AbortSignal): Promise<ProductRetail> {
  const query = supabase
    .from('product_retail')
    .select(`
      id,
  ...
  const productImages = ((row as any).product_retail_images || []) as { image_url: string; is_primary: boolean; display_order: number }[];
...
export function useProductRetailDetail(id: string | undefined) {
  ...
  return useQuery({
    queryKey: enabled ? ['productRetail', numericId] : ['productRetail', 'invalid'],
    enabled,
