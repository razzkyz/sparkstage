...
- **AI Background Removal Fix (NEW 2026-07-22)**: `docs/runbooks/AI-BACKGROUND-REMOVAL-FIX.md`
  - **Status:** ✅ PRODUCTION FIX APPLIED - READY TO DEPLOY
  - **Problem:** Feature tidak jalan di production dengan 3 critical errors
  - **Root causes identified:**
    1. WASM files loading from blob: URL instead of static files
    2. Cross-Origin-Isolation mode not enabled (missing COEP/COOP headers)
    3. Content-Security-Policy blocking blob: script sources
  - **Solutions applied:**
    1. vercel.json: Added WASM-specific headers (COEP, COOP, CORP, Content-Type)
    2. vercel.json: Updated CSP to allow blob: in script-src, connect-src, worker-src
    3. DevIDCardTest.tsx: Added explicit publicPath config + CPU mode
  - **Deploy guide:** `DEPLOY_NOW.md` ⭐ **READ THIS!**
  - **Technical details:** `PRODUCTION_FIX_WASM.md`
  - **Build status:** ✅ Verified - 2 WASM files (34.89 MB) in dist/
  - **Confidence:** 95% will fix production
  - **Next:** Commit → Push → Test production
  - Files modified: `vercel.json`, `DevIDCardTest.tsx`, `vite.config.ts`, `package.json`
