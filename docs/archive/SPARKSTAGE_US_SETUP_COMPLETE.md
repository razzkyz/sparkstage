...
Bagus! Anda sudah di track yang benar. Sekarang ikuti langkah ini:
...
- Clean git history
- Update package.json
- Install dependencies (termasuk Stripe)
- Create .env.local template
- Set dev port ke 5174

#### 2. Baca Panduan Lengkap
📖 **PENTING:** `.agents/skills/sparkstage-us-builder/SEPARATE_FOLDER_SETUP.md`

Panduan ini explain:
- Cara setup 2 folder terpisah
- Cara run Indonesia dan US secara bersamaan
- Database strategy (separate Supabase projects)
- Step-by-step lengkap

#### 3. Create US Supabase Project
```
...
#### 5. Start Development
```powershell
# Indonesia version (terminal 1)
cd C:\SparkDoku\sparkstage
npm run dev  # Port 5173

# US version (terminal 2)
cd C:\SparkDoku\sparkstageus
...
```
C:\SparkDoku\
├── sparkstage\          ← Indonesia (DOKU, RajaOngkir, port 5173)
│   └── .env.local       (DOKU credentials)
└── sparkstageus\        ← US (Stripe, EasyPost, port 5174)
    └── .env.local       (Stripe credentials - BEDA!)
```

### 📚 Documentation untuk Separate Folder:
- ⭐ `SEPARATE_FOLDER_SETUP.md` - Complete guide untuk 2 folder
- 🚀 `QUICK_COMMANDS.md` - Command reference (Indonesia vs US)
- 🗄️ `DATABASE_STRATEGY.md` - Database separation strategy
- 📖 `QUICKSTART_ID.md` - Step-by-step Bahasa Indonesia
...
**Status:** ✅ Ready to Use
**Total Files Created:** 
- 15 documentation files (.agents/skills/sparkstage-us-builder/)
- 3 setup scripts (separate folder + original setup)
- 1 env template
- 1 summary document
= **20 files total** 🎉
