...
async function releaseReservedProductResources(params: {
  ...
  for (const adjustment of params.reservedAdjustments) {
    const { error } = await params.supabase.rpc("release_retail_stock", {
      p_retail_id: adjustment.retailProductId,
...
serve(async (req) => {
  ...
  try {
    ...
    const normalizedItems: ProductItem[] = payload.items.map((i) => ({
      retailProductId: toNumber(i.retailProductId, 0),
    ...
    if (
      normalizedItems.some((i) => !i.retailProductId || !i.name || i.price < 0)
    ) {
    ...
    const aggregatedItemsByRetail = new Map<
      string,
      { retailProductId: number; name: string; quantity: number; sentPrice: number }
    >();
    for (const item of normalizedItems) {
      const key = `${item.retailProductId}_${item.price}`;
      const existing = aggregatedItemsByRetail.get(key);
      if (existing) {
        existing.quantity += item.quantity;
      } else {
        aggregatedItemsByRetail.set(key, {
    ...
    const aggregatedItems = Array.from(aggregatedItemsByRetail.values());
    const retailProductIds = aggregatedItems.map((item) => item.retailProductId);

    const { data: retailRows, error: retailsError } = await supabase
      .from("product_retail")
      .select("id, price, stock, is_active")
      .in("id", retailProductIds);

    if (retailsError || !Array.isArray(retailRows)) {
      return jsonError(req, 500, "Failed to load product variants");
    }

    console.log("[create-doku-product-checkout] Variant rows from DB:", JSON.stringify(retailRows));
    ...
    >();
    for (
      const row of retailRows as Array<
        {
          id: number;
          price: unknown;
          stock: unknown;
      ...
      retailMap.set(Number(row.id), row);
    }
    ...
    for (const item of aggregatedItems) {
      const variant = retailMap.get(item.retailProductId);
      if (!variant) {
        return jsonError(
          req,
          400,
          `Retail product not found: ${item.retailProductId}`,
        );
      }
      ...
      console.log(
        `[create-doku-product-checkout] Variant ${item.retailProductId}: sentPrice=${item.sentPrice}, dbPrice=${dbPrice}, unitPrice=${unitPrice}`
      );

      if (item.sentPrice === 0) {
        console.warn(
          `[create-doku-product-checkout] Variant ${item.retailProductId} sentPrice is 0, falling back to dbPrice: ${dbPrice}`
        );
      }
      ...
      if (unitPrice <= 0) {
        return jsonError(
          req,
          400,
          `Invalid price for variant: ${item.retailProductId}`,
        );
      }
    ...
    for (const item of resolvedItems) {
      const row = retailMap.get(item.retailProductId);
      if (!row) continue;
    ...
    for (const item of resolvedItems) {
      const row = retailMap.get(item.retailProductId);
      if (!row) {
        return jsonError(req, 400, "Retail product not found");
      }
      ...
      if (isActive === false) {
        return jsonError(
          req,
          400,
          `Retail product inactive: ${item.retailProductId}`,
        );
      }

      // Atomic reservation using RPC - prevents race conditions
      const { data: reserved, error: reserveError } = await supabase.rpc(
        "reserve_retail_stock",
        {
          p_retail_id: item.retailProductId,
    ...
    const lineItems = resolvedItems.map((item) => ({
      ...
      quantity: item.quantity,
      price: item.unitPrice,
      sku: sanitizeDokuString(`variant-${item.retailProductId}`, 64),
      category: "beauty",
      type: "PHYSICAL",
    }));
