-- Update save_inventory_product to preserve stock when not provided during updates
-- If variant stock is null/empty string on UPDATE → keep existing stock
-- If variant stock is null/empty string on INSERT → default to 0
...
create or replace function public.save_inventory_product(
  ...
  p_retail_category_id bigint default null,
  p_retail_subcategory_id bigint default null
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_product_id is null then
    insert into public.products (
      ...
      p_category_id,
      p_retail_category_id,
      p_retail_subcategory_id,
      btrim(p_sku),
    ...
    set name = btrim(p_name),
        ...
        retail_category_id = p_retail_category_id,
        retail_subcategory_id = p_retail_subcategory_id,
        sku = btrim(p_sku),
  ...
  if p_sync_variants then
    ...
    for v_variant in select * from jsonb_array_elements(coalesce(p_variants, '[]'::jsonb)) loop
      ...
      -- Extract stock value: null means "preserve existing"
      v_stock_raw := nullif(btrim(coalesce(v_variant->>'stock', '')), '');

      if v_variant_id is null then
        -- INSERT new variant: stock defaults to 0 if not provided
        insert into public.product_variants (
          ...
          coalesce(v_stock_raw::integer, 0),
          0,
          true,
          v_attributes,
          now()
        ...
        -- UPDATE existing variant: if stock is null, preserve existing stock
        if v_stock_raw is not null then
          ...
          -- stock not provided → keep existing stock untouched
          update public.product_variants
          set name = btrim(coalesce(v_variant->>'name', '')),
              sku = btrim(coalesce(v_variant->>'sku', '')),
              price = nullif(btrim(coalesce(v_variant->>'price', '')), '')::numeric,
...
revoke all on function public.save_inventory_product(
  text, text, text, boolean, bigint, text, bigint, jsonb, jsonb, text[], boolean, bigint, bigint
