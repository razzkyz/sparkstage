-- =====================================================
-- Migration: Add no-image filter support
-- Created: 2026-07-12
-- =====================================================
  ...
  WITH filtered_products AS (
    ...
    WHERE p.deleted_at IS NULL
      ...
      AND (
        ...
        OR (p_category_slug = 'no-image' AND NOT EXISTS (SELECT 1 FROM public.product_images pi WHERE pi.product_id = p.id))
        OR (p_category_slug = 'uncategorized' AND p.retail_category_id IS NULL)
        OR (p_category_slug LIKE 'rc-%' AND p.retail_category_id = (substring(p_category_slug from 4))::bigint)
